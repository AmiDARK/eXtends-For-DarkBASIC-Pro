...
;
  Global DSKInitialized.l

;
; *************************************************************************************** Start()
ProcedureCDLL AConstructor00YAXXZ()
  Retour.l = InitDLLBase()
...
ProcedureCDLL.s AGetDependencyID00YAPBDH0Z( NumID.l )
  Select NumID
    ...
    Case 1 : NewPTR = PeekS( ?DEP_DBPRO01 )      ; Include DarkBASIC Professional FILE
    Default : NewPTR = ""
   EndSelect
  ProcedureReturn NewPTR
 EndProcedure
; **************************************************************
ProcedureCDLL.l DSK_Init()
  If DSKInitialized = 0
    DSKInitialized = InitDLLBase()
   EndIf
  If *GlobPtr = 0 : InitialiseCorePtr() : EndIf
  ProcedureReturn DSKInitialized 
 EndProcedure
;
; ************************************************************** Files updated to handle : .s = MyDeleteString( .l )
; **************************************************************
  IncludeFile "DBProFileDynamic.pb"
; **************************************************************
  IncludeFile "DBProFileHighScores.pb"
; **************************************************************
  IncludeFile "DBProFileAdditional.pb"
; **************************************************************
DataSection
DEP_eXtends:
...
Data.s "DBProFileExtends.dll Ver 1.5 [ 2012.06.26 ]  / "
...
; FirstLine = 36
; Folding = -
; Executable = eXtends_FilesIO.dll
; DisableDebugger
; AddResource = File.rc